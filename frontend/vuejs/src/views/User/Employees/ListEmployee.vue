        ...
        <CCol sm="4">
          <div class="form-group">
            ...
            >
              ...
              <option
                :value="item"
                v-for="(item, index) in japaneseLevel"
                :key="index"
                >{{ $t(`commons.label_japanese_level_${item}`) }}</option
              >
            </select>
          </div>
        </CCol>
          ...
          <div class="form-group">
            ...
            >
              ...
              <option
                :value="item.code"
                v-show="
                  item.parent_code === searchData.footCode ||
                    searchData.footCode == 0
                "
          ...
          <CCardBody>
            <div class="option-table">
              <div class="form-group">
                ...
                >
                  <option
                    v-for="(item, index) in listItemsPerPage"
                    :value="item"
                    :key="index"
                    >{{ item }}</option
                  >
            ...
            <div class="table-responsive">
              <table class="table">
                <thead>
                  <tr>
                    <th>#</th>
                    <th>{{ $t("employee.label_employee_code") }}</th>
                    <th scope="col">
                    ...
                    <tr v-for="(item, index) in employees.data" :key="index">
                      ...
                      <td class="py-2 d-flex justify-content-center">
                        ...
                        <CButton
                          ...
                          square
                          size="sm"
                          @click.prevent="deleteEmp(item.id)"
                        >
...
export default {
  ...
  methods: {
    ...
    async deleteEmp(employeeId) {
      Swal.fire({
        title: this.$t("popup.label_popup_title"),
        text: this.$t("popup.label_popup_text"),
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#3085d6",
        cancelButtonColor: "#d33",
        confirmButtonText: this.$t("popup.label_confirm_button"),
        cancelButtonText: this.$t("popup.label_cancel_button")
      }).then(result => {
        if (result.value) {
          this.deleteEmployee(employeeId)
            .then(() => {
              ...
              Swal.fire(
                this.$t("popup.label_popup_success_title"),
                this.$t("popup.label_popup_success_text"),
                "success"
              );
            })
            .catch(() => {
              Swal.fire({
                icon: "error",
                text: this.$t("popup.label_popup_error_text")
              });
            });
        }
      });
