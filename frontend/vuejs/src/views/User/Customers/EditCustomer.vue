  ...
  <div class="app aside-menu-show">
    <aside class="aside-menu">
      ...
      <ValidationObserver v-slot="{ handleSubmit }" ref="formCustomer">
        <CForm @submit.prevent="handleSubmit(updateCustomer)">
          <div class="tab-content">
            <div
              ...
              role="tabpanel"
            >
              <CInput type="hidden" v-model="customer.id" />
              <CRow class="mb-4">
            ...
            >
              <CRow>
                ...
                <CCol md="5" sm="12" class="employees-form-offshore">
                  ...
                  <CRow class="mb-4">
                    <CCol sm="12" xl="3">
                      <h5 class="contract">{{ $t("customer.contract") }}</h5>
                    </CCol>
                    <CCol sm="12" xl="6">
                      <ValidationProvider
                        rules="numeric|max:20"
                        v-slot="{ errors }"
                        :name="$t('customer.contract')"
                      >
                        <CInput
                          class
                          type="text"
            ...
            >
              ...
              <div class="list-customer-information">
                <div
                  v-for="(item_employee,
                  index) in customer.customer_responsibilities"
                  :key="index"
                  class="customer-infomation mb-5"
                >
                  <div v-if="item_employee.type == 1">
                    <CRow class="mb-4">
                      <CCol sm="12" md="3" lg="3" xl="2">
                        <label>
                      ...
                      <CCol sm="12" md="7" xl="5">
                        <CInput type="text" v-model="item_employee.name" />
                      </CCol>
                      <span
                        @click="deleteInformation(index)"
                        v-if="checkTypeLength(1)"
                      >
                    ...
                    <CRow class="mb-4">
                      ...
                      <CCol sm="12" md="7" xl="5">
                        <CInput
                          class="-select"
                          type="text"
                          v-model="item_employee.position"
                        />
                      </CCol>
                    </CRow>
                  </div>
...
export default {
  ...
  methods: {
    ...mapActions("customer", ["edit"]),
    filterData(type) {
      if (this.customer) {
        return this.customer.customer_responsibilities.filter(
          item => item.type === type
        );
      }
    },
    ...
    validateForm() {
      let fields = [];
      this.isValid = true;
      for (let employee of this.customer.customer_responsibilities) {
    ...
    async updateCustomer() {
      this.validateForm();
      let form = JSON.parse(JSON.stringify(this.customer));
      ...
      let employeesType_1 = this.filterData(1);
      let employeesType_2 = this.filterData(2);
      let employeesType_3 = this.filterData(3);
      for (let i = 0; i < employeesType_1.length; i++) {
        let data = {
      ...
      for (let i = 0; i < employeesType_2.length; i++) {
        let data = {
          name: employeesType_2[i].name
        };
        form.responsibilityFabbiJapan.push(data);
      }
      for (let i = 0; i < employeesType_3.length; i++) {
        let data = {
          name: employeesType_3[i].name
        };
        form.responsibilityFabbi.push(data);
      }
      let check = await this.$refs.formCustomer.validate();
      if (check && this.isValid) {
        await this.$store.dispatch("customer/update", form).then(() => {
          let message = ["Updated"];
    ...
    checkTypeLength(type) {
      return (
        this.customer.customer_responsibilities.filter(
          item => item.type === type
        ).length > 1
      );
    },
