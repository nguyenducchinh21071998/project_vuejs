<template id="user-list">
  <div>
    ...
    <div style="border: 1px solid #e7ecf1; margin-left: 19px; height: 660px; margin-top: 24px;  width: 100%">
        ...
        <h2 style="margin-top: 37px; text-align: center;">Cập nhật thông tin</h2>
        <form v-on:submit="saveForm()" style="border: 1px solid white; width: 860px; margin-left: 99px;">
            <div class="form-group" >
                <label for="name">Name (<span style="color: red">*</span>)</label>
                <input type="text" id="name" v-model="company.name" class="form-control" placeholder="Nhập name">
                <!-- <span v-if="error && errors.name" style="color: red">{{ errors.name }}</span> -->
            </div>
            <div class="form-group" >
                <label for="email">E-mail (<span style="color: red">*</span>)</label>
                <input type="email" id="email" class="form-control" placeholder="Nhập user@example.com" v-model="company.email" >
                <!-- <span  v-if="error && errors.email" style="color: red">{{ errors.email }}</span> -->
            </div>  
            <div class="form-group" >
                <label for="mobile">Mobile (<span style="color: red">*</span>)</label>
                <input type="mobile" id="mobile" class="form-control" placeholder="Nhập số điện thoại" v-model="company.mobile" >
                <!-- <span  v-if="error && errors.mobile" style="color: red">{{ errors.mobile }}</span> -->
            </div>  

            <div class="form-group" >
                <label for="address">Địa chỉ (<span style="color: red">*</span>)</label>
                <input type="address" id="address" class="form-control" placeholder="Nhập địa chỉ" v-model="company.address" >
                <!-- <span  v-if="error && errors.address" style="color: red">{{ errors.address }}</span> -->
            </div>
            <button type="submit" class="btn btn-primary" style="text-align: center;">Cập nhật</button>
        </form>
    </div>
        <!-- /.content-wrapper -->
    ...
    export default {
        ...
        methods: {
            saveForm() {
                ...
                Axios.patch('/edit/' + app.companyId, newCompany)
                    .then(function (resp) {
